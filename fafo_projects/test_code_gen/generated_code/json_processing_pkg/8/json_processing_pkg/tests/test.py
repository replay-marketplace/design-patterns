...
class TestGettersSetters:
    ...
    def test_generate_json_schema_from_json(self):
        ...
        try:
            ...
            input_file = os.path.join(test_dir, "input.json")
            with open(input_file, 'w') as f:
                json.dump(test_data, f)
            
            assert generate_json_schema_from_json(input_file)
            
            schema_file = os.path.join(test_dir, "input_schema.json")
...
class TestAdvancedFunctions:
    ...
    def test_store_json_to_directory(self):
        ...
        test_file = os.path.join(self.test_dir, "test_root", "test.txt")
        assert os.path.exists(test_file)
        
        nested_file = os.path.join(self.test_dir, "test_root", "subdir", "nested.txt")
    ...
    def test_round_trip_directory_json(self):
        ...
        # Store to directory
        assert store_json_to_directory(original_json, self.test_dir)
        
        # Load back from directory
        loaded_json = load_directory_to_json(os.path.join(self.test_dir, "original"))
        
        assert loaded_json["type"] == "directory"
        assert loaded_json["name"] == "original"
        assert len(loaded_json["children"]) == 1
        assert loaded_json["children"][0]["name"] == "data.txt"
        assert loaded_json["children"][0]["content"] == "test data"
