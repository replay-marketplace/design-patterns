import unittest
from src.linear_search import linear_search

class TestLinearSearch(unittest.TestCase):
    """Test cases for linear_search function."""
    
    def test_target_found_at_beginning(self):
        """Test finding target at the beginning of the array."""
        arr = [1, 2, 3, 4, 5]
        self.assertEqual(linear_search(arr, 1), 0)
    
    def test_target_found_at_end(self):
        """Test finding target at the end of the array."""
        arr = [1, 2, 3, 4, 5]
        self.assertEqual(linear_search(arr, 5), 4)
    
    def test_target_found_in_middle(self):
        """Test finding target in the middle of the array."""
        arr = [1, 2, 3, 4, 5]
        self.assertEqual(linear_search(arr, 3), 2)
    
    def test_target_not_found(self):
        """Test when target is not in the array."""
        arr = [1, 2, 3, 4, 5]
        self.assertEqual(linear_search(arr, 10), -1)
    
    def test_empty_array(self):
        """Test searching in an empty array."""
        arr = []
        self.assertEqual(linear_search(arr, 1), -1)
    
    def test_single_element_found(self):
        """Test finding target in single element array."""
        arr = [42]
        self.assertEqual(linear_search(arr, 42), 0)
    
    def test_single_element_not_found(self):
        """Test not finding target in single element array."""
        arr = [42]
        self.assertEqual(linear_search(arr, 1), -1)
    
    def test_duplicate_elements(self):
        """Test array with duplicate elements returns first occurrence."""
        arr = [1, 2, 3, 2, 4]
        self.assertEqual(linear_search(arr, 2), 1)
    
    def test_string_elements(self):
        """Test searching for string elements."""
        arr = ['apple', 'banana', 'cherry']
        self.assertEqual(linear_search(arr, 'banana'), 1)
    
    def test_string_not_found(self):
        """Test string not found in array."""
        arr = ['apple', 'banana', 'cherry']
        self.assertEqual(linear_search(arr, 'grape'), -1)
    
    def test_mixed_types(self):
        """Test array with mixed types."""
        arr = [1, 'two', 3.0, None]
        self.assertEqual(linear_search(arr, 'two'), 1)
        self.assertEqual(linear_search(arr, None), 3)

if __name__ == '__main__':
    # Create test suite and run tests
    loader = unittest.TestLoader()
    suite = loader.loadTestsFromTestCase(TestLinearSearch)
    runner = unittest.TextTestRunner(verbosity=2)
    result = runner.run(suite)
    
    # Write test result to file
    with open('../replay/test_blind_bool.txt', 'w') as f:
        if result.wasSuccessful():
            f.write('PASSED')
        else:
            f.write('FAILED')
